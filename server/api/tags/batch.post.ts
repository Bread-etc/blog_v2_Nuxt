...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!action) {
      return createErrorResponse("操作类型是必填字段", 400, false);
    }

    if (!["create", "delete"].includes(action)) {
      return createErrorResponse(
        "无效的操作类型，支持: create, delete",
        400,
        false,
      );
    }
    ...
    if (!data || !Array.isArray(data) || data.length === 0) {
      return createErrorResponse("操作数据不能为空", 400, false);
    }
    ...
    return createErrorResponse("批量操作标签失败" + error, 500, false);
  }
});
...
async function batchCreateTags(tags: any[], currentUser: any) {
  ...
  try {
    // 使用事务批量创建
    const createdTags = await prisma.$transaction(async (tx: any) => {
      const results = [];
    ...
    return createSuccessResponse(
      ...
      `成功创建 ${createdTags.length} 个标签`,
      200,
    );
  } catch (error: any) {
    return createErrorResponse("批量创建标签失败" + error, 500, false);
  }
}
...
async function batchDeleteTags(tagIds: any[], currentUser: any) {
  ...
  if (validTagIds.length === 0) {
    return createErrorResponse("没有有效的标签ID", 400, false);
  }
  ...
  try {
    ...
    const usedTagIds = tagsInUse.map((pt: any) => pt.tagId);
    ...
    console.log(
      `用户 ${currentUser.userName} 批量删除了 ${deleteResult.count} 个标签`,
    );
    ...
    return createSuccessResponse(
      ...
      `成功删除 ${deleteResult.count} 个标签${usedTagIds.length > 0 ? `，跳过 ${usedTagIds.length} 个正在使用的标签` : ""}`,
      200,
    );
  } catch (error: any) {
    return createErrorResponse("批量删除标签失败" + error, 500, false);
  }
}
